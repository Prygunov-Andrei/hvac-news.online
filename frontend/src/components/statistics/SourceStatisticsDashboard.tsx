...
import referencesService, { StatisticsSummary } from '../../services/referencesService';
...
export default function SourceStatisticsDashboard() {
  ...
  const { overview, aggregated, averages, categories, top_sources } = statistics;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold">Статистика источников новостей</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Общий обзор эффективности и активности источников
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <MetricCard
          ...
          subtitle={`${overview.resources_with_stats} со статистикой`}
          icon={BookOpen}
          color="blue"
        />
        <MetricCard
          title="Активных источников"
          value={formatNumber(overview.active_resources)}
          subtitle={`${((overview.active_resources / overview.total_resources) * 100).toFixed(1)}% от общего числа`}
          icon={TrendingUp}
          color="green"
        />
        <MetricCard
      ...
      {/* Блок 3: Топ источников */}
      <div>
        <h2 className="text-xl font-semibold mb-4">Топ источников</h2>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <TopSourcesTable
            title="По количеству новостей"
            data={top_sources.by_news}
            type="news"
          />
          <TopSourcesTable
            title="По рейтингу"
            data={top_sources.by_ranking}
            type="ranking"
          />
          <TopSourcesTable
            title="По активности (30 дней)"
            data={top_sources.by_activity}
            type="activity"
          />
        </div>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                Всего ошибок: {formatNumber(aggregated.total_errors)}. Рекомендуется проверить проблемные источники.
              </p>
            </div>
          </div>
        </Card>

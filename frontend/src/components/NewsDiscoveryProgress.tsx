...
interface NewsDiscoveryProgressProps {
  onComplete?: (status: NewsDiscoveryStatus) => void;
  onError?: (error: string) => void;
}

export default function NewsDiscoveryProgress({ onComplete, onError }: NewsDiscoveryProgressProps) {
  const [status, setStatus] = useState<NewsDiscoveryStatus>({
    status: 'running',
    processed: 0,
    total: 0,
    percent: 0,
  ...
  const pollStatus = async () => {
    try {
      const newStatus = await referencesService.getNewsDiscoveryStatus();
      setStatus(newStatus);
      ...
      if (newStatus.status === 'completed' || newStatus.status === 'error') {
        ...
        if (newStatus.status === 'completed' && onComplete) {
          ...
          const errorMsg = newStatus.message || 'Произошла ошибка при поиске новостей';
          setError(errorMsg);
          if (onError) {
            onError(errorMsg);
          }
      ...
      if (!pollIntervalRef.current && newStatus.status === 'running') {
        pollIntervalRef.current = setInterval(async () => {
          try {
            const updatedStatus = await referencesService.getNewsDiscoveryStatus();
            ...
            if (updatedStatus.status === 'completed' || updatedStatus.status === 'error') {
              ...
              if (updatedStatus.status === 'completed' && onComplete) {
                ...
                const errorMsg = updatedStatus.message || 'Произошла ошибка при поиске новостей';
                setError(errorMsg);
                if (onError) {
                  onError(errorMsg);
                }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {getStatusIcon()}
            <h3 className="font-semibold">Поиск новостей</h3>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">
              Обработано {status.processed} из {status.total} источников
            </span>
